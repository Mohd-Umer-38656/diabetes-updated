...
export default function ProductPage() {
  ...
  const [mainImage, setMainImage] = useState(""); // Main image for the product
  const [userId, setUserId] = useState(null); // User ID state
  const [quantity, setQuantity] = useState(1); // Quantity selector state
  const [activeTab, setActiveTab] = useState("description"); // Tab selection state
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const storedProduct = sessionStorage.getItem("selectedProduct"); // Retrieve product from session storage
      if (storedProduct) {
  ...
  if (!product) return <p>Loading...</p>; // Handle case when product data is not available
  ...
  const addToCart = async (product) => {
    ...
    if (!token) {
      alert("Please log in first."); // Prompt user to log in
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/products") // Fetch related products from API
      .then((response) => {
  ...
  return (
    <div className="max-w-7xl w-[90%] md:w-[80%] lg:w-[80%] mx-auto  p-4 rounded-lg ">
      {/* Product Section */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
        <div className="flex gap-4 w-full md:w-full lg:w-45 items-start justify-center flex-col-reverse md:flex-col-reverse lg:flex-row">
          ...
          <div className="w-full md:w-full lg:w-16 flex flex-row md:flex-row lg:flex-col justify-center gap-2 pt-4 md:mt-0"></div>
        ...
        <div className="bg-white p-5 rounded-lg shadow-md">
          <div className="flex items-center gap-3">
            ...
            <span className="bg-green-100 text-green-600 relative left-[-20px] top-[-30px] px-3 py-1 text-[8px] md:text-[14px] rounded-md">
              {`In Stock`}
            </span>
          </div>
          ...
          <div className="flex items-center gap-4 mt-4 flex-col md:flex-row lg:flex-row">
            ...
            <div className="flex flex-row gap-2">
              <button
                ...
                className="bg-blue-400 text-[12px] md:text-[12px] lg:text-[14px] text-white px-4 py-2 md:px-6 md:py-2 rounded-full flex items-center gap-2 hover:bg-blue-300 transition md:w-auto"
              >
                Add to Cart{" "}
                <img
      ...
      <div className="mt-10 flex flex-col md:flex-row gap-6">
        <div className="flex-1 bg-gray-100 p-6 rounded-md mt-4">
          <div className="flex justify-center border-b pb-2">
            {["description", "additionalInfo", "customerFeedback"].map(
              (tab) => (
                <button
                  key={tab}
                  className={`px-4 py-2 font-semibold text-[8px] md:text-[14px] md:text-base ${
                    activeTab === tab
          ...
          {/* Tab Content */}
          <div className="mt-4 p-2 md:p-4  text-[8px] md:text-[14px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 mt-4">
          {relatedProducts.map((item) => (
            ...
            >
              ...
              <button
                onClick={() => addToCart(product)}
                className="absolute bottom-7 right-4 bg-blue-300 text-white p-2 rounded-full shadow-md hover:bg-blue-200 transition"
              >
                {" "}
                <img
                  src="/Icons & Icon-gifs/cart-static.png"
